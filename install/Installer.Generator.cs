...
public static class Generator
{
  ...
  public static WixEntity[] GenerateWixEntities( IEnumerable<string> args )
  {
    ...
    foreach ( var directory in args ) {
      ...
      var fileVersion = versionRegex.Match( directoryInfo.Name )
        .Value ;
      ...
      var files = new Files( feature,
        $@"{directory}\*.*",
        FilterEntities ) ;
      if ( versionStorages.TryGetValue( fileVersion,
            out var storage ) ) {
        storage.Add( files ) ;
      }
      else {
        versionStorages.Add( fileVersion,
          [files] ) ;
      }

      LogFeatureFiles( directory,
        fileVersion ) ;
    }

    return versionStorages.Select( storage => new Dir( new Id( $"INSTALL{storage.Key}" ),
        storage.Key,
        storage.Value.ToArray() ) )
      .Cast<WixEntity>()
      .ToArray() ;
  }
  ...
  /// </summary>
  private static void LogFeatureFiles( string directory,
    string fileVersion )
  {
