...
sealed partial class Build
{
      ...
      .Executes( () =>
      {
        foreach ( var project in Bundles ) {
          ...
          foreach ( var contentDirectory in targetDirectories ) {
            var version = YearRegex.Match( contentDirectory )
              .Value ;
  ...
  /// </summary>
  void GenerateManifest( Project project,
    string[] directories,
    AbsolutePath manifestDirectory )
  {
    BuilderUtils.Build<PackageContentsBuilder>( builder =>
    {
      ...
      var versions = directories
        .Select( path => YearRegex.Match( path )
          .Value )
        .Select( int.Parse ) ;
  ...
  /// </summary>
  string GetConfigurationValue( Project project,
    Func<XElement, bool> filter )
  {
    var defaultValue = string.Empty ;
    var configPath = project.Directory
      .GetFiles( "*.addin" )
      .FirstOrDefault() ;
    ...
    if ( configDocument.Root is null )
      return defaultValue ;

    var sectionElement = configDocument.Root
      .Elements()
      .FirstOrDefault() ;
    if ( sectionElement is null )
      return defaultValue ;

    var configElement = sectionElement.Elements()
  ...
  /// </summary>
  static void CopyAssemblies( string sourcePath,
    string targetPath )
  {
