...
sealed partial class Build
{
  ...
  Project[] Bundles =>
  [
    Solution.Sonny.RevitExtensions
  ] ;
  ...
  Dictionary<Project, Project> InstallersMap =>
    new() { { Solution.Automation.Installer, Solution.Sonny.RevitExtensions } } ;
  ...
  string PreviousReleaseVersion =>
    GitTasks.Git( "tag -l --sort=v:refname", logInvocation: false, logOutput: false )
        .ToArray() switch
      {
        ...
        _ => GitTasks.Git( "rev-list --max-parents=0 HEAD", logOutput: false, logInvocation: false )
          .First()
          .Text
      } ;
