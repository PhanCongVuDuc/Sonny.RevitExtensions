...
sealed partial class Build
{
      ...
      .Executes( () =>
      {
        ...
        foreach ( var (wixInstaller, wixTarget) in InstallersMap ) {
          ...
          var targetDirectories =
            Directory.GetDirectories( wixTarget.Directory, $"* {configuration} *", SearchOption.AllDirectories ) ;
          Assert.NotEmpty( targetDirectories, "No content were found to create an installer" ) ;

          var arguments = targetDirectories.Select( path => path.DoubleQuoteIfNeeded() )
            .JoinSpace() ;
          var process =
            ProcessTasks.StartProcess( builderFile, arguments, logInvocation: false, logger: InstallerLogger ) ;
  ...
  void InstallerLogger( OutputType outputType,
    string output )
  {
